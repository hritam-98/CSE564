...
var tempDims;
const colormap=["blue","yellow","red"];
...
function clearBoard() {
    ...
    d3.select("#plotcolorlegend").html("")
    d3.select("#heading_pcp_mds").html("")
}
...
function draw() {
    ...
    .enter().append('g')
    .attr('class', 'legend')
    .attr('transform', function(d, i) { return 'translate(' + (200) + ',' + (12 +i * 20) + ')'; });
    ...
    console.log(key_array_for_pcp)
    if (key_array_for_pcp.length !=0 ) {d3.select("#heading_pcp_mds").text("Parallel Coordinates Plot");}

}

function mds_pcp(props){

    if(mdsPcpButton==true){
    d3.json("/mdspcp").then(function(repos) {
        console.log("==================")
        console.log(repos)
        console.log("==================")
        ...
        create_dimensions_msdpcp(props);
